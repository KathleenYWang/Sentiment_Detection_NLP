...
FILL_VOCAB = config['train']['True']
BATCH_SIZE = config['train']['BATCH_SIZE']

SENT_HIDDEN_SIZE = config['train']['SENT_HIDDEN_SIZE']
CTX_LSTM_DIM = config['train']['CTX_LSTM_DIM']
...
GLOVE_EMB_PATHB = config['emb']['glove_path']
...
def main():
    ...
    final_test_data_set = create_data.TestDataSet(final_test_data_list, EMAI_PAD_LEN, SENT_PAD_LEN, word2id, id2word, use_unk=False)
    final_test_data_loader = DataLoader(final_test_data_set, batch_size= min(BATCH_SIZE, len(final_test_data_list)), shuffle=False)
