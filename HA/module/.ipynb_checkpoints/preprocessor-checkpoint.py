...
class EnglishPreProcessor(object):
    ...
    def remove_email_header(self,sentence):
        '''
        :param sentence:
        :return:
        '''
        # Remove email header
        sentence_repl = re.sub(r'[-]+ Forwarded by ((.|\n)*)(?<=--\n)', '', sentence, flags=re.IGNORECASE)
        sentence_repl = re.sub(r'[-]+Original Message[-]+\n*', '', sentence_repl, flags=re.IGNORECASE)
        sentence_repl = re.sub(r'(To|Cc|Bcc): ((.|\n)*)(?<=(Subject:))', r'\4', sentence_repl, flags=re.IGNORECASE)
        sentence_repl = re.sub(r'(From|Sent|Subject): .*\n*', '', sentence_repl, flags=re.IGNORECASE)
        return sentence_repl
    ...
    def remove_special_characters(self,sentence):
        ...
        sentence_repl = re.sub(r'(?<!\S)\*+(?!\S)\n*', '', sentence)
        sentence_repl = re.sub(r'(?<!\S)\-+(?!\S)\n*', '', sentence_repl)
        sentence_repl = re.sub(r'(?<!\S)_+(?!\S)\n*', '', sentence_repl)
        return sentence_repl
    ...
    def __call__(self, sentence):
        ...
        x = self.remove_email_header(x)
        x = self.remove_special_characters(x)
        return x
