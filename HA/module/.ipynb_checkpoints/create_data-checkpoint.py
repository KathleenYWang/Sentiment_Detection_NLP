...
def build_vocab(clean_sent_lists, vocab_size, fill_vocab=False):
    ...
    #limit the size of the vocab to 'vocab_size' to limit model size
...
class TrainDataSet(Dataset):
    def __init__(self, data_list, target_list, emai_pad_len, sent_pad_len, word2id, emoji_st, use_unk=False):
...
#         self.e_c_emo = []
    ...
    def sent_to_ids(self, sent_text):
        ...
        a_emoji = self.emoji_st.tokenize_sentences([sent_text])[0].reshape((-1)).astype(np.int64)
...
class TestDataSet(Dataset):
    ...
    def __init__(self, data_list,  emai_pad_len, sent_pad_len,  word2id, id2word, emoji_st, use_unk=False):
