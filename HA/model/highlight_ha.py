...
class HierarchicalAttPredictorHL(nn.Module):
    """
    ...
    def __init__(self, embedding_dim, hidden_dim_s, hidden_dim_e , vocab_size, SENT_PAD_LEN, id2word, USE_ELMO, ADD_LINEAR):
        super(HierarchicalAttPredictorHL, self).__init__()
...
#         emb_x = emb_x[:, :max_len, :]
    ...
    def forward(self, list_of_a, list_of_a_len, list_of_a_emoji,  elmo_a=None):        
        ...
        if isinstance(self.ctx_self_attention, AttentionOneParaPerChan):
            ctx_out, ctx_weight = self.ctx_self_attention(ctx_out, torch.LongTensor([EMAI_PAD_LEN for _ in range(NUM_OF_ROWS)]))
        else:
            ctx_out, ctx_weight = self.ctx_self_attention(ctx_out)
            ctx_out = ctx_out[:, 0, :]
